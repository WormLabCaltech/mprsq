...
quantitatively measure gene expression level. There is widespread interest in
using transcriptomic profiles as a
phenotype for genetic analysis. However, a source of concern is that
whole-animal transcriptomic profiles mix the expression signatures of multiple
cellular states, which might make it hard to accurately reconstruct genetic
interactions. Additionally, it has been difficult to measure
epistasis, the signature of genetic interaction between two genes, using
expression profiles. Here, we show that it is possible to accurately
reconstruct genetic interactions between genes using whole-animal RNA
sequencing, and we demonstrate a way to quantify epistasis transcriptome-wide.
With the advent of genome
engineering tools, mutants can be created quickly and easily in many
organisms. As a result, phenotyping mutants is the rate-limiting step towards
reconstructing genetic interaction networks. Our work potentially represents a
solution to this problem because transcriptome profiling is sensitive
to a broad variety of genetic perturbations.
}
...
this concept has not been applied to whole-organism RNA-seq. Also,
...
epistasis that reflects the underlying interactions and which can be interpreted
...
generate quantitative genetic models of any kind.
\color{purple}
In quantitative genetics, eQTL
