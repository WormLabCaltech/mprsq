...
Transcriptome profiling quickly and quantitatively measures gene expression
level. There is widespread interest in using transcriptomic profiles as a
phenotype for epistasis analysis. Although epistasis measurements between two
genes can be performed for every transcript in a transcriptome, this results in
thousands of epistasis measurements that must be interpreted independently. We
developed a statistic that summarizes these measurements and makes
interpretation of this data straightforward. Moreover, epistasis analysis has
previously only been performed on cDNA extracted from single cells. We show that
whole-organism RNA-seq, like single-cell RNA-seq, can be used to characterize
interactions between two genes. With the advent of genome engineering tools,
mutants can be created quickly and easily in many organisms. As a result,
phenotyping mutants is now the rate-limiting step in building interaction
networks. Our work potentially represents a solution to this problem because
transcriptome profiling is sensitive to a broad variety of genetic perturbations.
}
...
\begin{abstract}
  ...
  expression which have opposite changes in expression in mutants of two genes
...
studying all genes in parallel. In particular, RNA-seq~\cite{Mortazavi2008}
...
RNA-seq has been used to identify genetic modules involved in a variety of
processes, such as in the \emph{Caenorhabditis~elegans} linker cell
migration~\cite{Schwarz2012}, planarian stem cell
maintenance~\cite{VanWolfswinkel2014,Scimone2014}. The role of transcriptional
...
quantitative genetic models of any kind. \color{purple} In quantitative
genetics, eQTL studies have established the power of transcriptomes for genetic
...
\color{black}
Epistasis analysis of single cells or single-celled organisms is popular because
of the concern that whole-organism sequencing will mix information from multiple
cell types, preventing the accurate reconstruction of genetic interactions.
Using whole-organism transcriptome profiling, we have recently identified a new
...
hypoxia pathway~\cite{Epstein2001,Shen2006,Shao2009,Jiang2001}.
...
$s({\text{\gene{egl-9}, \gene{hif-1}}}) = -0.80\pm0.01$ (see SI Figure 2), and we found
...
Millard and Muriel Jacobs Genetics and Genomics Laboratory at Caltech. Strains
were provided by the CGC, which is funded by the NIH Office of Research
Infrastructure Programs (P40 OD010440). We thank Hillel Schwartz, Erich Schwarz,
Jonathan Liu, Han Wang and Porfirio Quintero for their advice throughout this
project.}
