...
\section*{Abstract}
% \textbf{Analysis of transcriptomes (RNA-seq) is commonly used to identify genetic
% modules that respond to perturbations.
...
% mutants. Interpreting these high-dimensional measurements is unintuitive.
% We thus developed a single coefficient to quantify transcriptome-wide epistasis
% that accurately reflects the underlying interactions. To demonstrate the power
% of our approach, we sequenced four single and two double
% \emph{Caenorhabditis~elegans} mutants. From these mutants, we successfully
% reconstructed the known hypoxia pathway that was derived from classic double mutant
% analyses in which \eglp{} and \vhlp{} inhibit the activity of \hifp{}. In addition,
% we uncovered a class of 31 genes that have opposing changes in expression in
% \egl{} and \vhl{} but for which the \eglvhl{} mutant has the same phenotype as \egl{}.
...
% transcriptomic phenotypes  have previously been used by to make genetic inferences.
...
% transcriptomes for population genetics studies~\cite{Brem2002,Schadt2003,
% Li2006,King2014}.
% In cell culture, single-cell RNA-seq has seen significant
...
% \textbf{B}. An example of read quantification using Kallisto for a particular
% isoform (top panel) and differential expression quantification using Sleuth
% (bottom panel).
}
\label{fig:explain}
\end{figure}
...
\gene{egl-9}. We propose in the~\nameref{sec:discussion} a model for how \hifp{}
