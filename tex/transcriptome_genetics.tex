...
\newcommand{\eglvhl}{\emph{\mbox{egl-9;vhl-1}}(lf)}
\newcommand{\eglhif}{\emph{\mbox{egl-9}~\mbox{hif-1}}(lf)}
...
\newcommand{\vhltargets}{45} % 44 genes, minus vhl-1 itself (IDed due to deletion)
...
genes and extracted mRNA from all mutants. BW made the libraries. DA developed
the genetic theory. DA wrote all computer code and performed all analyses.
DA, CPR and PWS wrote the manuscript.
}

\authordeclaration{The authors declare no conflict of interest.}
\equalauthors{\textsuperscript{*}DA and CPR
...
\egl{} (\emph{sa307}), \rhy{} (\emph{ok1402}), \vhl{} (\emph{ok161}), \hif{} (\emph{ia4}). We also sequenced
the transcriptomes of two double mutants, \eglvhl{} (\emph{sa307}, \emph{ok161}) and
\eglhif{} (\emph{sa307}, \emph{ia4}) as well as wild-type N2 as a control sample. Each genotype
 was sequenced in triplicate at a depth of 15 million reads.
We performed whole-organism RNA-seq of these mutants at a moderate
...
previously~\cite{Angeles-Albores2016a}. Since \gene{fog-2} has not been described
...
phenotype (SRP)---a set of genes or isoforms that are differentially expressed in
...
measured phenotype; and if there is epistasis we must measure whether it is
...
% As a
% first step towards reconstructing a pathway, we defined transcriptomic phenotypes
% as a set of genes or isoforms that are shared between two genes when these become
% mutated.
% Importantly, shared transcriptomic phenotypes are not directional---genes that
% are commonly altered between mutants need not be regulated in the same direction.
...
number of isoforms within the SRP and
...
% Taken together, these results indicate that RNA-seq data is
...
In other words, the each individual isoform's x-coordinate is the sum of the regression
...
If two genes interact in a single, unbranched pathway, then $a^-$ and $b^-$ should
...
regression on to the two gene combinations we studied (\gene{egl-9} and
...
line); whereas
genes that act along a single unbranched pathway will have $s=-1/2$ (blue line).
Strong
...
% priori} reason why global gene expression should reflect order,
% the strength of the unweighted correlations between genes in the hypoxia pathway
% made us wonder how much information can be extracted from this dataset.
...
all \egl{} genotypes. We were did not find any genes that
met these conditions.
% I don't really know if we need this next sentence.
% If we relaxed our conditions substantially by identifying
% genes that changed expression in \gene{egl-9}$^-$ genotypes and which did not
% change expression in the same direction in \vhl{} mutants, we found \egltargets{}
% genes (see Supplementary Information).
% vhl dependent response
...
for genes that are direct candidates.  We found \hiftargets{}  genes which
consistently increase expression in mutants with a constitutive hypoxic response.
...
\gene{ogt-1}, an O-linked GlcNac Transferase gene; \gene{T04A8.7}, encoding an ortholog
of human glucosidase, acid beta (GBA); and \gene{T22F3.3}, the gene for the
...
% Complex II may play a similar role in \cel{}.
...
% Arguments for biological activity of \hifp{}-hydroxyl have not yet been made
% although experts have wondered about
...
% Third, although deletion of
% \gene{vhl-1} and \gene{egl-9} lead to similar \hifp{} expression levels, the Egl
% phenotype of \vhl{} is considerably attenuated---in fact, \gene{vhl-1} is not
% annotated with an egg-laying defective phenotype in WormBase (version WS256).
...
mechanisms that are robust to transient changes in protein copy number. Likewise,
...
existence of systematic outliers. Moreover, primary and secondary correlations
must be performed to measure the strength of both interactions that are present.
...
% ZG31 \gene{hif-1}(\emph{ia4}) is a null mutant of \gene{hif-1} which deletes
% 1231bp of the second, third and fourth exons.
% JT307 is a homozygote of the null mutant \gene{egl-9}(\emph{sa307}) which is a
% 243bp deletion.
% RB1297 contains null mutation \gene{rhy-1}(\emph{ok1402}) with an estimated 700bp
% deletion
% constructed by the OMRF Knockout Group. CB5602 contains the deletion mutation of
% \gene{vhl-1}(\emph{ok161}).
% CB6088 contains \gene{egl-9}(\emph{sa307})~\gene{hif-1}(\emph{ia4}).
% CB6116 contains \gene{egl-9}(\emph{sa307});\gene{vhl-1}(\emph{ok161}).
...
This article wouldn't be possible without help from Dr. Igor Antoshechkin who
