...
predict the slope that results for each epistatic combination ($X^-Y^-=X^-$ or
$X^-Y^-=Y^-$). Given the biological relevance of the slope of the lines of best
fit to the biological relationship between the genes under study, we refer to it
as the genome-wide epistasis coefficient, because it integrates information from
many different genes into a single number.
...
should have if \gene{egl-9} was epistatic to \gene{vhl-1} ($s_\mathrm{Pred}=-0..42$)
...
slope agreed almost exactly with the predicted slope ($s_\mathrm{Obs}=-0.42$)
(see Fig.~\ref{fig:egl9epistasis}). The epistasis coefficient is close to $-0.5$,
which suggests that \gene{egl-9} and \gene{vhl-1} interact in a linear pathway.
However, the epistasis coefficient for this gene pair has negligible probability
density near $-0.5$ and the distribution is sharply peaked around $-0.42$. A
genetic pattern that can lead to epistasis coefficients between $-\frac{1}{2}$ and
$0$ is a branched pattern, where the gene that is most upstream interacts with
the output phenotype via two arrows of the same valency. Biologically, this
means that \eglp{} inhibits \hifp{} in \gene{vhl-1}-dependent and independent
manners~\cite{Shao2009}.
...
\caption{
The \gene{egl-9} is epistatic over \gene{vhl-1}. (\textbf{A}) Epistasis plot showing
that the \eglvhl{} transcriptome deviates significantly from a null additive model
and approaches slopes typical for genes that interact in a linear pathway. Points
are colored qualitatively according to density (purple---low density, yellow---high
density) and their size is inversely proportional to the standard error of the
y-axis (larger points, more accurate measurement). In red is the line of best fit
from an orthogonal distance regression. (\textbf{B}) Probability density plot
showing predicted slopes if \gene{vhl-1} was epistatic over \gene{egl-9}; if
\gene{egl-9} was epistatic over \gene{vhl-1}; and the observed slope. Slopes
that have significant density around the point $-0.5$ are indicative of genes
in a linear pathway. The observed slope is consistent with a \gene{egl-9} acting
via \gene{vhl-1}-dependent and independent pathways.
